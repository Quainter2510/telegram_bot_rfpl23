...
def main_table(table, tour):
    img = Image.open("table_templates/template_main_table.png")
    imdraw = ImageDraw.Draw(img)
    dx = [44, 336, 395, 452, 510, 568, 626, 682, 739, 799, 855, 913, 970, 1031, 1090, 1146, 1205, 1280]
    font = ImageFont.truetype("Fonts/consolas.ttf", size=28)
    for i in range(14):
    ...
    img.save("main_table.png")


def result_tour(res, tour, points, nickname=""):
    img = Image.open("table_templates/template_result_tour.png")
    imdraw = ImageDraw.Draw(img)
    ...
    for i in range(8):
        for j in range(4):
            ...
            imdraw.text((dx[j] + (dx[j + 1] - dx[j] - w) // 2, 140 + 65 * i + (65 - h) // 2), msg,
                        font=font)
    img.save("result_tour.png")


def points_tour(data, tour):
    img = Image.open("table_templates/template_points_tour.png")
    imdraw = ImageDraw.Draw(img)
